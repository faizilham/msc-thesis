...
*Proof.* Based on Lemma 1, if $abs(sigma) >= 2$, all values in $sigma$ must be non-occluded construction calls. There are two cases of $e$. The first case is that $e$ is an expression label. Using Lemma 2 and the fact that $sigma subset.eq r_e$ we conclude that all values in $sigma$ exist exclusively. The second case is that if $e$ is a variable. We restate the Lemma 3 in its contraposition form, that is if a construction call $f$ is not included in occluded set $o_e$, there is no $f, g in r_e$ such that node $q_f$ is an ancestor to node $q_g$. Since any value in $sigma$ is not occluded and $sigma subset.eq r_e$, each value in $sigma$ is not an ancestor to the others and thus exists exclusively. $qed$
]
