...
we shall prove that the set $sigma = "SafeReach"(p, e)$ is safely reachable for any $p$ and $e$, that is if $abs(sigma) >=2$, all values in $sigma$ are construction calls that are alive exclusively from each other. Construction calls $f$ and $g$ are exclusively alive if their CFG nodes $mono(p_f: lbl(e_f) = f(...))$ and $mono(p_g: lbl(e_g) = g(...))$ are not the ancestor of each other. The set of ancestors of a node is defined as follows, given $"pred"(p)$ the direct predecessor of node $p$.
...
*Lemma 2*. For any expression label $e$, node $p$, and $(r_e, o_e) = evalexit(p)(e)$, if the reachable definitions set $r_e$ has more than one construction calls, all of them must be alive exclusively to each other.\
*Proof.* Let $f$ and $g$ besome construction calls in $r_e$. Construction calls are only added to the reachable definition set of an expression label in case of function calls. Hence, there must be some nodes $q_f: e = f(...)$ and $q_g: e = g(...)$ which are ancestors of $p$. By the definition of the control flow graph, the value expression nodes must have expression label that is unique to the program path. Since both $q_f$ and $q_g$ have the expression label $e$, the nodes $q_f$ and $q_g$ must appear in different program path. This means that $q_f$ and $q_g$ cannot be the ancestor of each other. Therefore, $f$ and $g$ must be alive exclusively to each other. $qed$
...
*Proof.* Let the node $q_f$ and $q_g$ be respectively $q_f: lbl(e_f) = f(...)$ and $q_g : lbl(e_g) = g(...)$. The reachable set $r_x$ is set in case of variable declaration and assignment, while the  occluded set $o_x$ is only updated in the latter case. Since $f, g in r_x$ and $q_f$ is ancestor to $q_g$, there must be a variable declaration or assignment node to $x$ in the form of $p_f:(mono("var"))x:=lbl(e_f)$, which is an ancestor to an assignment node $p_g: x := lbl(e_g)$. The node $p_g$ must be an assignment node because a variable declaration node must not have ancestors which are declaration or assignment node of the same variable. Both $p_f$ and $p_g$ are ancestors of $p$, and since $f in r_x$, there must also be a program path from $p_f$ to $p$ that do not include $p_g$, otherwise the assignment of $x$ to $f$ is always overwritten to $g$. Node $p$ is also neither a declaration nor an assignment to $x$ for the same reason.
...
#[
  ...
  If between node $p_f$ and $p_g$ there are no assignment node of variable $x$, then $rg supset.eq rf supset.eq {f}$. If there is an assignment node $p_h$ such that $(rh, oh) = evalexit(p_h)(x)$, then $og supset.eq oh supset.eq rf supset.eq {f}$. This also inductively applies if there are more assignment nodes between them. because an assignment to $x$. Since either $f in rg$ or $f in og$ applies, then $f in ogx$. The node $p_g$ is an ancestor to $p$, therefore $ogx supset.eq o_x$ and $f in o_x$. $qed$
]

Finally, we prove the safely-reachable property.

*Theorem*. Given $sigma = "SafeReach"(p, e)$, all values in $sigma$ are exclusively alive if $abs(sigma) >=2$.\
*Proof.* Based on Lemma 1, if $abs(sigma) >= 2$, all values in $sigma$ must be non-occluded construction calls. There are two cases of $e$. The first case is that $e$ is an expression label. Using Lemma 2 and the fact that $sigma subset.eq r_e$ we can conclude that all values in $sigma$ are exclusively alive. The second case is that if $e$ is a variable. We restate the Lemma 3 in its contraposition form, that is if a construction call $f$ is not included in occluded set $o_e$, there is no $f, g in r_e$ such that node $q_f$ is an ancestor to node $q_g$. Since any value in $sigma$ is not occluded and $sigma subset.eq r_e$, each value in $sigma$ is not an ancestor to the others, and thus is alive exclusively. $qed$
]
