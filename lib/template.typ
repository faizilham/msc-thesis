...
#let in-tail = state("in-tail", false)
...
#let template(
  ...
  set page(
    ...
    // NOTE: for alternating page number
    // footer: context {
  ...
  show heading.where(level: 1): it => {
    let chapter_number = if heading.numbering == none {
        []
      } else {
        it.supplement + " " + counter(heading.where(level: 1)).display() + "\n"
      }
    ...
    header-ascent: 30%, header: context{
      ...
      if before.len() > 0 {
        ...
        if current.numbering != none {
          ...
          let (left-text, right-text) = (chapter-number-text, chapter-title)

          text(fill-line(left-text, right-text))
          v(-1em)
