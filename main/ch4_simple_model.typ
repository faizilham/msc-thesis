...
+ The same case also applies to the calls $C_6$ and $C_7$.

There are two ways to approach the problem. The first one is to trace paths backward from a `utilize` call to some `create` calls. The second one is to trace paths forward from a `create` call to some `utilize` calls.
...
The next case is the variable access expression `$e = x`, where the utilization of expression label $e$ is propagated to variable $x$. We use meet operation to make sure that if $x$ is already utilized in the current path, it should remain so. The last cases are the variable declaration and assignment `(var) x := $e`. It is quite similar to the variable access expression, but with resetting the utilization of $x$ to $top$. This is because any values assigned prior to this node are "hidden" by the current assignment and cannot be traced to any future `utilize`, therefore the utilization status is unknown.
...
The SafeReach function returns the reachable value set if it only contains a single value. Otherwise, it returns non-occluded construction sites. The Sources function merely resolves a singleton set of variable reference $(x, p')$ into its safely-reachable set. The soundness of our overall analysis depends on the correctness of safely-reachable property. This is because later in the utilization analysis, the analysis determines which value should be marked as utilized based on the Sources function, which depends on SafeReach function. The set #box($sigma = "SafeReach"(p, e)$) is safely reachable if either there is only at most one reference ($abs(sigma) < 2$) or all references in $sigma$ are construction calls that exist exclusively.

We provide the full proof of this property in @apx:SafeReachProof. In short, we prove that if there are two construction calls that do not exist exclusively, such that one call's node is an ancestor to the other, the ancestor call must be included in the occluded set. Since the result of SafeReach always excludes the occluded set if there is more than one reachable definition, its members cannot be ancestors to each other and thus exist exclusively to each other.
...
We use a quite different lattice than the one in the backward analysis, as shown in @eq:ForwardLattice. Instead of a linearly-ordered lattice of $(bot, "UT", top)$, the utilization lattice $U$ is a flat lattice with the partial ordering #box($bot #h(5pt) leqsq NU, UT leqsq top$). The lattice $U$ has four values: $top$ which means "maybe utilized or not" or "unknown", $UT$ which means "definitely utilized" (equivalent to UT), $NU$ which means "definitely not utilized" and $bot$ which means "not created" or "not accessible". Currently, the utilization status $NU$ is not very useful, but this will become useful when we include utilization prerequisites later. The program state lattice $S$ is a mapping from the construction calls instead of references since any variables and other expressions are resolved with the Sources function.
