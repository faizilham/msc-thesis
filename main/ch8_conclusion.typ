...
To summarize our research, we present a data flow analysis for tracking the utilization of values, and implement it as a Kotlin compiler plugin. Using Kotlin's annotation feature, we extend the function signature with utilization and effect notation. Our analysis can produce warnings on unutilized values, check the correct utilization value and effect for each function call, and perform a basic utilization and effect annotation inference for lambda functions.
...
Lastly, while our analysis mainly handles utilization tracking, it is also possible to model a simple linear resources such as a file handler. With more works, the utilization value and effect annotation could potentially be generalized to include linear and affine types. An algebra of the utilization values and effect may also be defined to notate more specific cases.
