...
+ infers a safe, over-approximation of the utilization status of each utilizable value, and produces warnings for any values with incomplete utilization,
...
Lastly, while our analysis mainly handles utilization tracking, it is also possible to model simple linear resources such as a file handler. With more work, the utilization status and effect annotation could potentially be *generalized to include the other substructural type systems*, such as the linear and affine types. An algebra of the utilization status and effects may also be defined to notate more specific cases.
