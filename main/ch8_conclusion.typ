...
The utilization and effect annotation for the function signature are also quite complex to read and write, especially for the parametric ones. This is because we implement the notation using Kotlin's annotation feature, and thus we are limited by its syntax. This may be improved by either developing a more concise syntax, or improving the inference technique to allow functions to be fully inferred without user-provided annotations.
