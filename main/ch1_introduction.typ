...
Programmers sometimes ignore the return value of a function call when writing a piece of code. While sometimes this might be a deliberate choice, it is often an oversight that may lead to bugs. In most cases, using a value means passing it as an argument to a function or as a return value of the enclosing function. However, for some types of values, it is more useful to define its usage as passing the value to a certain set of functions, and not necessarily any functions. We call this kind of usage a utilization, and the types of values are utilizable types.

For example, the `Deferred` type in Kotlin represents asynchronous calls, which can be awaited using the `.await()` method, suspending the computation until the call is finished, or canceled by using the `.cancel()` method.
In most cases, `Deferred` values should be eventually awaited or canceled. Otherwise, it is potentially a source of bugs that should be warned to users. Therefore, the `Deferred` type is a utilizable type, which must be utilized by the methods `await` or `cancel`, or any user-defined functions that call those methods upon its `Deferred` parameters.

@lst:UtilOfDeferred shows an example of tracking the utilization of `Deferred` values.  The `Deferred` values in the variables `one` and `two` are always awaited or canceled, thus no warning is produced. However, the `Deferred` value in the variable `three` is not fully utilized since there is an execution branch where it is never awaited or canceled, thus a warning should be given about it.
...
Another example in Kotlin is the `Sequence` type, which represents lazy computations over collections of values, such as lists or sets. The `Sequence` type is commonly used in Kotlin programs for efficiently chaining collection computations, such as maps and filters, and executing it all at once. However, sometimes users forget to collect the result of the sequence, and as a result, the computations are never executed. A `Sequence` value is utilized if its computations are eventually executed and collected as a new collection, for example using the `toList` function.

Using the utilization model, we may also model safe usages of types that represent linear resources, such as a file handler. Suppose that a file handler type `File` has four built-in functions: `open()` for opening a new file, `read(f)` for reading the content of file `f`, `write(f,s)` for writing the value `s` to the file `f`, and `close(f)` for closing the file `f`. We define an opened file handler as having an unutilized status, while a closed one as having a utilized status. The `open()` function always creates a new unutilized file handler. The `read(f)` and `write(f,s)` functions require the parameter `f` to be unutilized, i.e. opened, without changing its utilization status. The `close(f)` function also requires `f` to be unutilized, but then as a side effect it would close the file and utilize the file handler value.

== Problem definition

We define the challenges in utilization tracking by looking at common value utilization patterns found in Kotlin code. As we explained earlier, a value of utilizable type must be utilized by passing it as an argument to a set of utilizing functions or returning it as the result of the enclosing function. These utilizing functions consist of some built-in utilizing functions or methods, such as `await` and `cancel` in the case of a `Deferred` type, and any user-defined functions that passed its parameter to the built-in functions or other utilizing functions. Therefore, we shall provide a way to *annotate a user-defined function as a utilizing function* and *check if it correctly utilizes its argument(s)*.

Many Kotlin code bases we found use the `Deferred` type through a combination of higher-order functions, lambda expressions, and collections or lists of `Deferred` type. @lst:DeferredLambdaCollection illustrates the common patterns we found.
...
+ User-defined functions can be annotated with utilization status requirements and effects, and the analysis shall check the annotations' correctness.
+ The analysis shall infer the utilization status and effects of lambda functions.
+ The analysis shall track utilization through type-parametric higher-order functions and collection types.
+ The analysis shall soundly handle the reference alias problem.
+ The analysis is compatible with the Kotlin compiler framework, which means that it is based on intra-procedural data-flow analysis and does not introduce new syntax.
